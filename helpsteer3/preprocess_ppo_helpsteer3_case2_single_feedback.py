...
def select_random_feedback(feedback_list: List[str]) -> str:
    """Randomly select one feedback item from the feedback list"""
...
def make_map_fn(split: str, data_source: str = "helpsteer3_feedback", seed: int = 42):
    """
    Create mapping function for Case 2 (Single Feedback Variant): 
    Same Conversation + Previous Response + ONE Random Feedback (Single-Turn)
    
    Similar to the original Case 2, but instead of using all feedback items,
    we randomly select ONE feedback item from the list.
    
    For each example, randomly choose response1+feedback1 OR response2+feedback2,
    then randomly select ONE feedback item from the chosen feedback list.
    """
    def process_fn(example: Dict, idx: int) -> Dict:
        ...
        if random.random() < 0.5:
            ...
            chosen_feedback_list = feedback1
            chosen_response_id = 1
        else:
            chosen_response = response2
            chosen_feedback_list = feedback2
            chosen_response_id = 2
        
        # **NEW**: Select ONE random feedback from the list
        selected_feedback = select_random_feedback(chosen_feedback_list)
        
        # Case 2 (Single Feedback): Single-turn prompt with previous response to SAME context
        # Uses only ONE randomly selected feedback item
        prompt_content = (
            f"Here is a previous response to this conversation with feedback:\n\n"
            f"Conversation:\n{formatted_context}\n\n"
            f"Previous Response: {chosen_response}\n\n"
            f"Feedback: {selected_feedback}\n\n"
        ...
        data = {
            ...
            "extra_info": {
                "split": split,
                "index": idx,
                "case": "with_previous_response_and_single_feedback",
                "domain": domain,
                "language": language,
                "conversation_length": len(context),
                "chosen_response_id": chosen_response_id,
                "previous_response": chosen_response,
                "all_feedback": chosen_feedback_list,
                "selected_feedback": selected_feedback,
                "feedback_list_size": len(chosen_feedback_list) if chosen_feedback_list else 0,
            },
        }
        return data
...
def preprocess_dataset(
    ...
    """Preprocess HelpSteer3 feedback data for Case 2 (Single Feedback Variant)"""
    ...
    print(f"Processing {len(raw_data)} examples for Case 2 (Single Feedback)...")
    ...
    print(f"  Split: {split}")
    print(f"  Case: With Previous Response and Single Feedback (Randomly Selected)")
    ...
    # Statistics on feedback list sizes
    avg_feedback_size = df['extra_info'].apply(lambda x: x['feedback_list_size']).mean()
    print(f"  Average feedback list size: {avg_feedback_size:.1f}")
    
    # Show example
    print(f"\nExample prompt (with previous response to same conversation + one random feedback):")
    example_prompt = df['prompt'].iloc[0]['content']
    # Show first 600 chars
    print(f"  {example_prompt[:600]}...")
    print(f"\nPrompt structure: Previous Response to SAME Conversation + ONE Random Feedback → Respond to Conversation Again")


def main():
    parser = argparse.ArgumentParser(
        description="Preprocess HelpSteer3 feedback data for PPO training (Case 2: Single Feedback Variant)"
    )
    ...
    parser.add_argument(
        "--local_save_dir",
        type=str,
        default="~/data/helpsteer3_ppo/case2_single_feedback",
